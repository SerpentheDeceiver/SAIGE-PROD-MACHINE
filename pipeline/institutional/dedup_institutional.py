...
- No changes to the actual dedup logic (exact-hash + adaptive soft-dedup by
  embedding similarity) - that logic was not the source of the data loss;
  confirmed by the fact that last run's report showed 0 exact and 0 soft
  duplicates removed on whatever (undersized) input it actually received.
"""
import json
import hashlib
import sys
...
def main():
    ...
    report = {
        ...
        "notes": [
            ...
            "Soft dedup embeds (title + content) using MiniLM cosine similarity.",
